...
export default function ProductUpdateScreen() {
  ...
  const [selectedImageUri, setSelectedImageUri] = useState<string | null>(
    productToEdit?.productImage || null
  );
  const [supplier, setSupplier] = useState(productToEdit?.supplier?.name || '');
  const [price, setPrice] = useState(productToEdit?.price?.toString() || '');
  const [category, setCategory] = useState(productToEdit?.category?.name || '');
  const [quantity, setQuantity] = useState(productToEdit?.amount?.toString() || '');
  const [expirationDate, setExpirationDate] = useState(
    productToEdit?.expirationDate ? new Date(productToEdit.expirationDate).toLocaleDateString('pt-BR') : ''
  );
  const [barcode] = useState(productToEdit?.barCode || '');
  ...
  const handleUpdateProduct = () => {
    ...
    const updatedProduct: Product = {
      ...productToEdit, // Mantém o ID original e outras propriedades não editáveis
      productName,
      description,
      ...
      manufacturingDate: productToEdit.manufacturingDate,
          ...
          <ScrollView contentContainerStyle={styles.container}>
            ...
            <View style={styles.barcodeInputContainer}>
              <TextInput
                ...
                placeholderTextColor="#999"
                value={barcode}
                editable={false} // Barcode geralmente não é editável após o cadastro
              />
              <MaterialIcons name="qr-code-scanner" size={24} color="#666" style={styles.barcodeIcon} />
            </View>

            <FormActionButtons onSave={handleUpdateProduct} onCancel={handleCancel} saveText="ATUALIZAR" />
          </ScrollView>
        </TouchableWithoutFeedback>
      </SafeAreaView>
...
const styles = StyleSheet.create({
  ...
  // Você pode ter styles para os botões dentro de FormActionButtons
});