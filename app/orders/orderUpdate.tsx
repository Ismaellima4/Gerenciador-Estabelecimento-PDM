...
import {
  View, Text, ScrollView, Modal, Alert, SafeAreaView, TextInput,
  Pressable, TouchableOpacity, KeyboardAvoidingView, Platform, Keyboard, StyleSheet,
} from 'react-native';
...
import { addOrderItem, deleteOrderItemById, resetOrderItems, setOrderItems } from '@/store/orderItemSlice';
import { findOrderById, updateOrder } from '@/store/orderSlice';
...
export default function OrderUpdate() {
  ...
  const order = useSelector((state: RootState) =>
     findOrderById(state, orderId as string)
  );
  ...
  const handleUpdateOrder = async () => {
    ...
    if (orderItems.length === 0) {
      return Alert.alert('Erro', 'Adicione ao menos um item ao pedido.');
    }
        ...
        <Pressable onPress={Keyboard.dismiss} style={{ flex: 1 }}>
          <ScrollView contentContainerStyle={{ padding: 20 }} keyboardShouldPersistTaps="handled">
            <Text style={registerStyles.label}>
              Atualizar Pedido <Text style={styles.required}>*</Text>
            </Text>
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 20 }}>
              <Text style={registerStyles.label}>Itens do Pedido</Text>
            ...
            <GreenButton title="Atualizar Pedido" onPress={handleUpdateOrder} />
          </ScrollView>
          ...
          <Modal visible={productModalVisible} animationType="slide">
            <SafeAreaView style={{ flex: 1 }}>
              <View style={{ padding: 20, flex: 1 }}>
                ...
                <ScrollView>
                  {products.filter((item) => item.amount > 0).map((item) => (
                    <Pressable
                      key={item.id}
                      onPress={() => {
...
// 🔁 Reutilize o mesmo objeto `styles` da tela de cadastro
